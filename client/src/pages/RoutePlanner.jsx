...
const RoutePlanner = () => {
  ...
  useEffect(() => {
    ...
    async function bootstrapMeta() {
      try {
        ...
        if (!mounted) return;
        setTransports(transportResponse?.data?.data?.length ? transportResponse.data.data : FALLBACK_TRANSPORTS);
  ...
  const loadPlaces = useCallback(async () => {
    ...
    try {
      ...
      setPlacesError(error?.response?.data?.message || "Unable to load places for this area.");
    } finally {
  ...
  const calculateFare = useCallback(
    async (route) => {
      ...
      try {
        ...
        setFareError(error?.response?.data?.message || "Unable to calculate fare right now.");
      } finally {
  ...
  const fetchRoute = useCallback(
    async (destination) => {
      if (!userLocation) {
        setRouteError("Allow location access first to compute routes from your position.");
        return;
      }
      ...
      try {
        const response = await requestWithRetry(
          () =>
            routeAPI.computeRoute({
              ...
              optimization: optimizationMode === "custom" ? "balanced" : optimizationMode,
        ...
        setRouteResult(data);
        setSelectedRouteId(data?.bestRouteId || data?.routes?.[0]?.routeId || "");
      } catch (error) {
        setRouteError(error?.response?.data?.message || "Unable to compute route at the moment.");
      } finally {
    ...
    <section className="pt-20 md:pt-24 pb-8 bg-slate-100">
      <div className="mx-auto max-w-[1600px] px-4 md:px-6">
        <div className="mb-4 rounded-2xl bg-white shadow-sm border border-slate-200 px-5 py-4">
          <h1 className="text-xl md:text-2xl font-semibold text-slate-900">Map and Route Planner</h1>
          <p className="text-sm text-slate-600 mt-1">
            Live location, clustered places, optimized route selection, and dynamic fare estimation.
          </p>
        </div>
        ...
        <div className="grid gap-4 lg:grid-cols-[360px_minmax(0,1fr)]">
          <aside className="rounded-2xl bg-white border border-slate-200 shadow-sm p-4 space-y-4 h-fit">
            <div className="space-y-2">
              <p className="text-sm font-medium text-slate-700">Route Strategy</p>
              <select
            ...
            {optimizationMode === "custom" ? (
              <div className="space-y-3">
                <p className="text-sm font-medium text-slate-700">Custom Weights</p>
                {["distanceWeight", "timeWeight", "costWeight"].map((key) => (
                  <label key={key} className="block">
                    <span className="text-xs text-slate-600 capitalize">{key.replace("Weight", "")}</span>
                    <input
                      type="number"
                      min="0"
                      max="10"
            ...
            {selectedPlace ? (
              ...
              >
                {routeLoading ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <RefreshCw className="h-4 w-4" />}
                Recalculate Route
              </button>
            ) : null}

            <div className="rounded-xl bg-slate-50 border border-slate-200 p-3">
              <p className="text-xs font-semibold text-slate-700 uppercase tracking-wide mb-1">Status</p>
              ...
              {locationError ? <p className="text-xs text-rose-600 mt-1">{locationError}</p> : null}
              {placesLoading ? (
                <p className="text-xs text-slate-500 mt-1">Refreshing visible map markers...</p>
              ) : (
                <p className="text-xs text-slate-500 mt-1">{places.length} places loaded in current map window.</p>
              )}
            </div>
            ...
            {selectedRoute ? (
              <div className="rounded-xl bg-teal-50 border border-teal-200 p-3 space-y-2">
                ...
                <p className="text-sm text-teal-900">Distance: {formatDistance(selectedRoute.distanceMeters)}</p>
                <p className="text-sm text-teal-900">Duration: {formatDuration(selectedRoute.durationSeconds)}</p>
                <p className="text-sm text-teal-900">Score: {Number(selectedRoute.score || 0).toFixed(2)}</p>
              </div>
            ) : null}
            ...
            <div className="rounded-xl bg-white border border-slate-200 p-3 space-y-1">
              ...
              {fareLoading ? <p className="text-sm text-slate-600">Calculating fare...</p> : null}
              {fareError ? <p className="text-sm text-rose-600">{fareError}</p> : null}
              {fareEstimate ? (
                <>
                  <p className="text-sm text-slate-700">Total fare: {formatCurrency(fareEstimate.totalFare)}</p>
                  <p className="text-sm text-slate-700">
                    Cost per person: {formatCurrency(fareEstimate.costPerPerson)}
                  </p>
                  <p className="text-sm text-slate-700">
                    Estimated time: {formatDuration(fareEstimate.estimatedTravelTimeSeconds)}
                  </p>
                </>
              ) : (
                <p className="text-sm text-slate-500">Choose a destination to view fare breakdown.</p>
              )}
            </div>

            <div className="rounded-xl bg-white border border-slate-200 p-3">
              <p className="text-sm font-semibold text-slate-800 mb-2">Popular Routes</p>
              <div className="space-y-2 max-h-44 overflow-auto pr-1">
                {popularRoutes.length ? (
                  popularRoutes.map((popular) => (
                    <div key={popular.id} className="rounded-lg border border-slate-100 px-2 py-2 bg-slate-50">
                      <p className="text-xs text-slate-700">Used {popular.usageCount} times</p>
                      <p className="text-xs text-slate-500">
                        {formatDistance(popular.distanceMeters)} • {formatDuration(popular.durationSeconds)}
                      </p>
                    </div>
                  ))
                ) : (
                  <p className="text-xs text-slate-500">No popularity data yet.</p>
                )}
              </div>
            </div>
          ...
          <div className="rounded-2xl border border-slate-200 overflow-hidden shadow-sm bg-white">
            <div className="h-[calc(100vh-10rem)] min-h-[480px]">
              <MapContainer center={DEFAULT_CENTER} zoom={13} className="h-full w-full" preferCanvas>
                ...
                {selectedPlace ? (
                  <CircleMarker
                    ...
                    radius={8}
                    pathOptions={{ color: "#dc2626", fillColor: "#f87171", fillOpacity: 0.85 }}
                  >
                ...
                {routeResult?.routes?.map((route) => {
                  ...
                  const positions = route.polyline?.length ? route.polyline : geometryToPolyline(route.geometry);
                  return (
                    <Polyline
                      key={route.routeId}
                      positions={positions}
                      pathOptions={{
                        color: route.isBest ? "#0f766e" : isSelected ? "#f59e0b" : "#64748b",
                ...
                <MarkerClusterGroup chunkedLoading maxClusterRadius={40} spiderfyOnMaxZoom>
                  {places.map((place) => (
                    <Marker key={place.id} position={[place.latitude, place.longitude]}>
                      <Popup>
                        <div className="space-y-2 min-w-[200px]">
                          <p className="font-semibold text-slate-800">{place.name}</p>
                          <p className="text-sm text-slate-600 capitalize">Type: {place.type}</p>
                          <p className="text-sm text-slate-700">Price: {formatCurrency(place.price)}</p>
                          <p className="text-xs text-slate-500">{place.details || "No details available."}</p>
                          <button
                            type="button"
